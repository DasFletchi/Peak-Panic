
### GENERAL
- funny. vibrant, nicht zu ernst nehmend gehalten
- american ahhh prisons weil die die main audience ist
- hohe Sättigung
- kaum Farbverläufe

### Die 1-Millisekunden-Regel
Wenn jemand durch TikTok scrollt, muss er SOFORT erkennen:
✅ Gefängnis
✅ Flucht
✅ Chaos
✅ Mehrere Spieler

Nicht
> "Ist das Roblox?"  
> "Ist das Ready or Not?"  
> "Ist das ein Unity-Template?"

***DARF NICHT ZU COPYCAT SEIN***
### CHARACTERS
- MÜSSEN in der ersten Sekunde INSTANT recognizeable sein (Silhuette MUSS stimmen)
- Orangle/Schwarz weisse kleidung? (wenn schwarz weiss dann muss der spieler sonst wo krass raustechen)

- Sträflingskleidung (Klassisch Orange oder Schwarz-Weiß gestreift) (an Robbery Bob (mobile Game) angelehnt?                                                                                                                                                    
- Jedes Outfit braucht ein klares visuelles Gimmick (z.B. riesige goldene Ketten, Kronen, Tattoos im Face, kaputte Masken) (earnbare cosmetics?)


**3D Silhuette für Inmate**
- Großer Kopf oder sehr markante Frisur/Kopfbedeckung, damit man sie auf einen Blick von den Wachen unterscheidet.

***Formen der Silhouette für Inmate***
- Etwas gebückte Haltung (Ready for action)                                                                                                                                                      
- Breiter, kompakter Oberkörper
- Handschellen-Reste an den HandgElenk als Cosmetic?
- sehr dünn und schlagsick weil GefängnisEssenRationen und so
  




***Guard***
- Grose schulter weil autorität
- big ass helm
- helle taschenlampen?
- kleiner Kopf



## ANIMATIONS
Jede Animation braucht Persönlichkeit.
Zum Beispiel:
Laufen
↓
Schultern wippen.

Werkzeug ziehen
↓
übertrieben schnell.

Tür eintreten
↓
ganzer Körper geht mit.

Erwischt werden
↓
Charakter friert eine Sekunde ein.

*Maybe Ragdoll/Procedural? weil Bug = funny? (wenn nicht Gamebreaking)*

### NOCH 1 IKONISCHES DING WO MAN INSTANT INSTANT WEISS "ja alr dass is Peak Panic xD"

### Licht
Nicht realistisch.

Eher:

- kräftige Sonnenstrahlen
- kaltes Neon
- rote Alarmlichter

Im Alarmmodus sollte jeder sofort denken:
> "fuck Jetzt eskaliert's."

---

# Visuelle Sprache
Alles kommuniziert Gameplay.
*Beispiele:*
Tür?
→ Riesiger aufmacher

Kaputt?
→ Risse.

Loot?
→ Leuchtet leicht.

Wache?
→ Kräftige Taschenlampe.

Keine unnötigen Details wie bars oder sowas, das gilt auch hier eigentlich für so ziemlich jedes game weniger UI == besser.

# LESBARKEIT AUS DISTANZ

Test:
Wenn man die Figur als schwarzen Schatten siehst:

- erkennst du „Guard“ sofort?
- erkennst du Typ?


### MAYBE?? (Guard Archetype)
### 1. „THE BRICK“
- alles breit, schwer, langsam
- T-Form

### 2. „THE POLE“
- dünn, hoch, schnell
- I-Form

### 3. „THE SHIELD“
- breite Frontfläche
- asymmetrisch

### 4. „THE SIGNAL“
- kleine Figur + riesiges Gerät oben
- „?“-Form


KONTRAST ZU PLAYERN

Wenn Guards blocky sind:
→ Spieler müssen **weniger massiv** sein

Wenn Guards vertikal sind
→ Spieler eher kompakt & chaotic

---

# TL;DR

Die Guard-Silhouette muss:
- sofort lesbar sein (1 Blick = Gefahr erkannt)
- klare Form-Archtypen haben (Brick / Pole / Shield / Signal)
- Helm als Identität nutzen (kein Gesicht nötig)
- starke negative spaces haben (kein Blob-Matsch)
- ein ikonisches funktionales Element besitzen (Licht, Schild, Antenne etc.)
- aus der Distanz erkennbar bleiben